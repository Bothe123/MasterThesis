...
\cite{radons2018}, 
$\gamma > 2(\nu - \eta)$, which in the picture corresponds to the region below a diagonal line parallel to the bold one whose offset is given by $2\eta$. Increasing $\eta$ therefore increases the domain of finite MSD in the picture, with $\nu=\eta$ always resulting in a finite MSD (since $\gamma >0$). From this we can see that for the original model with $\eta =1$ the Richardson regime is found either over or exactly on the line and is therefore divergent. 

These results where tested through simulation, with an ensemble of $10^5$ particles. To ensure that the particles had time to reach the asymptotic behavior of $t>>t_0$ the last observation time was set to $t_{max} = 20000 \; t_0$
...
The verification via the simulation now involved the fitting of the MSD with regards to the observation time as well as the aging time. Compared to the ordinary case considerably longer simulation times where need, as both $t>> t_0$ and $t_a>> t$ had to be satisfied to reach the asymptotic limit. For an ensemble of $10^5$ walkers with $t_{max}=5\; 10^{3} \; t_0$ and $t_{a,max} = 10^{6} \; t_0$ the aging time exponent is in good agreement with  the analytical prediction as can be seen in Fig. \ref{fig:plotMSDaged}.
%
\begin{figure}
\begin{center}
...
Increasing the value of $\gamma$ reduces the influence of very long stretches on the overall distribution, which manifests itself in less pronounced peaks and a \gls*{PDF} that is more concentrated around the origin, which is illustrated in Fig. \ref{fig:histogramAgedGammaLarge}.
